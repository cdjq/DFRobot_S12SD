# -*- coding: utf-8 -*
'''!
  @file       DFRobot_S12SD.py
  @brief       This is basic library of S12SD sensor
...
class DFRobot_S12SD():
  ...
  def read_UV_data(self):
    ...
    if self._uart_i2c == self.I2C_MODE:
      buffer = self._read_reg(self.S12SD_INPUTREG_UVS_DATA,2)
      data = buffer[0]|buffer[1]<<8
    else:
      buffer = self._read_reg(self.S12SD_INPUTREG_UVS_DATA,1)
      data = buffer[0]
    return data
  ...
  def read_UV_index(self):
    ...
    '''
    if self._uart_i2c == self.I2C_MODE:
      buffer = self._read_reg(self.S12SD_INPUTREG_UVS_INDEX,2)
      data = buffer[0]|buffer[1]<<8
    else:
      buffer = self._read_reg(self.S12SD_INPUTREG_UVS_INDEX,1)
      data = buffer[0]
    return data  
  ...
  def read_risk_level(self):
    ...
    '''
    if self._uart_i2c == self.I2C_MODE:
      buffer = self._read_reg(self.S12SD_INPUTREG_RISK_LEVEL,2)
      data = buffer[0]|buffer[1]<<8
    else:
      buffer = self._read_reg(self.S12SD_INPUTREG_RISK_LEVEL,1)
      data = buffer[0]
    return data

class DFRobot_S12SD_I2C(DFRobot_S12SD):
  '''!
    @brief An example of an i2c interface module
  '''
  def __init__(self ,bus):
    self._addr = self.S12SD_DEVICE_ADDR
...
class DFRobot_S12SD_UART(DFRobot_S12SD):
  '''!
